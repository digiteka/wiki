<template lang="pug">
  div
    .pa-3.d-flex(v-if='navMode === `MIXED`', :class='$vuetify.theme.dark ? `grey darken-5` : `dtk-dark-blue darken-3`')
      v-btn(
        depressed
        :color='$vuetify.theme.dark ? `grey darken-4` : `dtk-primary-blue darken-2`'
        style='min-width:0;'
        @click='goHome'
    ...
    v-list.py-2(v-if='currentMode === `custom`', dense, :class='color', :dark='dark')
      template(v-for='item of items')
        v-list-item(
          ...
          :load="log(item)"
          :class='{active:currentUrl === item.t}'
          )
