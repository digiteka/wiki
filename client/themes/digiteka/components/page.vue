<template lang="pug">
  v-app(v-scroll='upBtnScroll', :dark='$vuetify.theme.dark', :class='$vuetify.rtl ? `is-rtl` : `is-ltr`', class='dtk')
    nav-header-custom(v-if='!printView')
    v-navigation-drawer(
    ...
    v-main(ref='content')
      template(v-if='path !== `home`')
        v-toolbar(:color='$vuetify.theme.dark ? `grey darken-4-d3` : `grey lighten-3`', flat, dense, v-if='$vuetify.breakpoint.smAndUp', class="dtk-breadcrumb")
      ...
      v-container.dtk-dark-blue.pa-0(fluid, :class='$vuetify.theme.dark ? `darken-4-l3` : `lighten-4`')
        v-row(no-gutters, align-content='center', style='padding:20px 0;')
          v-col.page-col-content.is-page-header(style='margin-top: auto; margin-bottom: auto;', :class='$vuetify.rtl ? `pr-4` : `pl-4`')
            h1.headline(:class='$vuetify.theme.dark ? `text--lighten-2` : `text--darken-3`') {{title}}
            .caption.grey--text.text--darken-1 {{description}}
      v-divider
      v-container.pl-5.pt-4(fluid, grid-list-xl)
        v-layout(row)
          v-flex.page-col-sd.dtk-flex-order(lg3, xl2, v-if='$vuetify.breakpoint.lgAndUp')
            v-card.mb-5(v-if='tocDecoded.length', outlined)
              .overline.pa-5.pb-0(:class='$vuetify.theme.dark ? `blue--text text--lighten-2` : `dtk-primary-blue--text`') {{$t('common:page.toc')}}
            ...
            v-card.mb-5(v-if='tags.length > 0', outlined)
              .pa-5
                .overline.dtk-teal--text.pb-2(:class='$vuetify.theme.dark ? `text--lighten-3` : ``') {{$t('common:page.tags')}}
                v-chip.mr-1.mb-1.dtk-chip(
                  label
                  :color='$vuetify.theme.dark ? `dtk-teal darken-1` : `dtk-teal lighten-5`'
                  v-for='(tag, idx) in tags'
                  :href='`/t/` + tag.tag'
                  :key='`tag-` + tag.tag'
                  )
                  v-icon(:color='$vuetify.theme.dark ? `dtk-teal lighten-3` : `dtk-teal`', left, small) mdi-tag
                  span(:class='$vuetify.theme.dark ? `dtk-teal--text text--lighten-5` : `dtk-teal--text text--darken-2`') {{tag.title}}
                v-chip.mr-1.mb-1.dtk-chip(
                  label
                  :color='$vuetify.theme.dark ? `dtk-teal darken-1` : `dtk-teal lighten-5`'
                  ...
                  v-icon(:color='$vuetify.theme.dark ? `dtk-teal lighten-3` : `dtk-teal`', size='20') mdi-tag-multiple
            ...
            v-card.mb-5(outlined)
              .pa-5
                .overline.dtk-primary-blue--text.d-flex(:class='$vuetify.theme.dark ? `text--lighten-3` : ``')
            ...
            v-card(flat)
              v-toolbar(:color='$vuetify.theme.dark ? `dtk-dark-blue darken-4-d3` : `dtk-dark-blue lighten-3`', flat, dense)
                v-spacer
                v-tooltip(bottom)
                  template(v-slot:activator='{ on }')
                    v-btn(icon, tile, v-on='on', :aria-label='$t(`common:page.bookmark`)'): v-icon(color='#2f45b5') mdi-bookmark
                  span {{$t('common:page.bookmark')}}
                v-menu(offset-y, bottom, min-width='300')
                  template(v-slot:activator='{ on: menu }')
                    v-tooltip(bottom)
                      template(v-slot:activator='{ on: tooltip }')
                        v-btn(icon, tile, v-on='{ ...menu, ...tooltip }', :aria-label='$t(`common:page.share`)'): v-icon(color='#2f45b5') mdi-share-variant
                ...
                v-tooltip(bottom)
                  template(v-slot:activator='{ on }')
                    v-btn(icon, tile, v-on='on', @click='print', :aria-label='$t(`common:page.printFormat`)')
                      v-icon(color='#2f45b5') mdi-printer
                  span {{$t('common:page.printFormat')}}
                v-spacer

          v-flex.page-col-content.dtk-flex-order(xs12, lg9, xl10)
