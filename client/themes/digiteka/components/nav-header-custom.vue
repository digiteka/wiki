<template lang='pug'>
  v-app-bar.nav-header(color='black', dark, app, :clipped-left='!$vuetify.rtl', :clipped-right='$vuetify.rtl', fixed, flat, :extended='searchIsShown && $vuetify.breakpoint.smAndDown')
    v-toolbar(color='#000B46', flat, slot='extension', v-if='searchIsShown && $vuetify.breakpoint.smAndDown')
      v-text-field(
    ...
    v-layout(row)
      v-flex(xs5, md4)
        v-toolbar.nav-header-inner(color='black', dark, flat, :class='$vuetify.rtl ? `pr-3` : `pl-3`')
          img.org-logo(src='/_assets/img/logo_digiteka_fond_couleur.svg', style='max-width:100px;', @click='goHome', :alt='title', :title='title')
          ...
          //-         v-list-item-subtitle.overline.grey--text.text--lighten-2 Coming soon
          v-toolbar-title(:class='{ "mx-3": $vuetify.breakpoint.mdAndUp, "mx-1": $vuetify.breakpoint.smAndDown }', class="dtk-toolbar-title-subheading")
            span.subheading Support
                ...
                v-tooltip(bottom)
                  template(v-slot:activator='{ on: tooltip }')
                    v-btn(
                      ...
                      v-icon(color='#fff') mdi-file-document-edit-outline
            ...
            v-tooltip(bottom)
              template(v-slot:activator='{ on }')
                v-btn(icon, tile, height='64', v-on='on', @click='pageNew', :aria-label='$t(`common:header.newPage`)')
                  v-icon(color='#fff') mdi-text-box-plus-outline
              ...
              v-tooltip(bottom)
                template(v-slot:activator='{ on: tooltip }')
                  v-btn(
                    ...
                    v-icon(v-if='picture.kind === `initials`', color='#fff') mdi-account-circle
          ...
          v-tooltip(v-else, left)
            template(v-slot:activator='{ on }')
              v-btn(icon, v-on='on', color='grey darken-3', href='/login', :aria-label='$t(`common:header.login`)')
                v-icon(color='#fff') mdi-account-circle
